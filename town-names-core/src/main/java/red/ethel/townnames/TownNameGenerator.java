...
import red.ethel.townnames.builtin.BuiltinSet;
import red.ethel.townnames.engine.GrfNameSource;
import red.ethel.townnames.engine.NameGenerationEngine;
import red.ethel.townnames.engine.NameSource;
import red.ethel.townnames.grf.GrfParser;
    ...
    public static final class Builder {
        ...
        public BuildResult build() {
            ...
            for (Path path : filePaths) {
                try {
                    var sources = GrfParser.parse(path).stream()
                            .map(GrfNameSource::new)
                            .toList();
                    if (sources.isEmpty()) {
                        skipped.add(path);
                    } else {
