...
class TownNameGeneratorTest {
  ...
  void builderWithNoBundledOrFileSourcesFails() {
    assertThrows(IllegalArgumentException.class, () -> TownNameGenerator.builder().build());
  }
  ...
  void generatesNonEmptyNameFromFile() throws Exception {
    ...
    Path testGrf = Path.of(TownNameGeneratorTest.class.getResource("/test-names.grf").toURI());
  ...
  void badFileSourceIsSkipped() throws Exception {
    Path good = Path.of(TownNameGeneratorTest.class.getResource("/test-names.grf").toURI());
  ...
  void allBadSourcesThrows() {
    Path bad = Path.of("/nonexistent/path.grf");
    assertThrows(
        IllegalArgumentException.class, () -> TownNameGenerator.builder().withGrf(bad).build());
  }
  ...
  void generateBulkEquivalentToSequential() throws Exception {
    ...
    assertEquals(
        g.generate(5),
        List.of(h.generate(), h.generate(), h.generate(), h.generate(), h.generate()));
  }
}
